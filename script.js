...
const CONFIG = {
    ...
    // Sidebar
 //   SIDEBAR_EXPANDED: '23%',
 //   SIDEBAR_COLLAPSED: '11%',
 //   SIDEBAR_TABLET_COLLAPSED: '8%',
...
class SidebarManager {
    ...
    static toggleDesktop() {
        ...
        if (this.toggler) {
            this.toggler.setAttribute('aria-expanded', isNowExpanded);
        }
        
        document.dispatchEvent(new CustomEvent('sidebarToggle', { 
            detail: { collapsed: !isNowExpanded }
        }));
...
// In TooltipManager class, update updateTooltipBehavior()
static updateTooltipBehavior() {
    const isTablet = window.innerWidth <= CONFIG.TABLET_BREAKPOINT && 
                    window.innerWidth > CONFIG.MOBILE_BREAKPOINT;
    const isDesktopCollapsed = window.innerWidth > CONFIG.TABLET_BREAKPOINT && 
    ...
    static getTooltipText(element) {
        return element.dataset.tooltip || 
               element.querySelector('.nav-text, .contact-text')?.textContent?.trim() ||
...
class PortfolioHierarchyManager {
    ...
    static switchCategory(category, save = false) {
        ...
        // Handle branding subnav visibility
        this.toggleBrandingSubnav(isBranding);
        
        // Reset branding subnav when leaving branding section
        if (wasBranding && !isBranding) {
            this.resetBrandingSubnav();
            
            // Save current active subcategory before resetting
            const activeItem = DomUtils.$('.scroll-nav-item.active');
            if (activeItem) {
                this.saveState('lastBrandingSubcategory', activeItem.dataset.subcategory);
            }
        }
        ...
        if (save) {
            this.saveState('category', category);
        }
    }
    ...
    static toggleBrandingSubnav(show) {
        ...
        if (show) {
            ...
            // Load last active subcategory or default to stationary
            const lastSubcategory = this.loadState('lastBrandingSubcategory') || 'stationary';
            this.switchBrandingSubcategory(lastSubcategory, false);
        } else {
            // Hide branding subnav
            this.brandingSubnavWrapper.style.opacity = '0';
            this.brandingSubnavWrapper.style.transform = 'translateY(-10px)';
            this.brandingSubnavWrapper.style.transition = 'opacity 0.3s ease, transform 0.3s ease';
    ...
    static switchBrandingSubcategory(subcategory, save = false) {
        // Update branding subcategory content
        ...
        // Update branding subnav buttons
        this.scrollNavItems.forEach(item => {
        ...
        if (save) {
            this.saveState('lastBrandingSubcategory', subcategory);
        }
    }

    static resetBrandingSubnav() {
        // Reset all branding subnav buttons to inactive state
        this.scrollNavItems.forEach(item => {
        ...
        // Reset all branding subcategories to hidden
        this.brandingSubcategories.forEach(sub => {
            sub.hidden = true;
        ...
        // Set first subnav item as default for next time
        if (this.scrollNavItems.length > 0) {
            this.scrollNavItems[0].setAttribute('tabindex', '0');
        }
        
        // Save default state
        this.saveState('lastBrandingSubcategory', 'stationary');
    }

    static setupKeyboardNavigation() {
        // Main category keyboard navigation
        document.addEventListener('keydown', (e) => {
            ...
            // Branding subnav keyboard navigation
    ...
    static saveState(key, value) {
        try {
            ...
            console.warn('Could not save portfolio hierarchy state');
        }
    }
    ...
    static loadState(key) {
        try {
            ...
            console.warn('Could not load portfolio hierarchy state');
            return null;
        }
    }
    ...
    static loadState() {
        // Load main category
        const savedCategory = this.loadState('category') || 'data-analysis';
        this.switchCategory(savedCategory, false);
        
        // Load branding subcategory if needed
        if (savedCategory === 'branding') {
            const savedSubcategory = this.loadState('lastBrandingSubcategory') || 'stationary';
            this.switchBrandingSubcategory(savedSubcategory, false);
        }
    }
}

// ===== PORTFOLIO FILTER MANAGER (DEPRECATED - USE HIERARCHY MANAGER INSTEAD) =====
class PortfolioFilterManager {
    static init() {
        console.warn('PortfolioFilterManager is deprecated. Use PortfolioHierarchyManager instead.');
        PortfolioHierarchyManager.init();
    }

    static switchCategory(category, save = false) {
        PortfolioHierarchyManager.switchCategory(category, save);
    }
}
...
class SkillsAnimationManager {
    static init() {
        ...
        skillLevels.forEach(level => {
            if (DomUtils.isElementInViewport(level, 0.3)) {
                this.animateSkill(level);
            }
        });
    }
...
class ContactFormManager {
    ...
    static setupAutoSave() {
        this.form.addEventListener('input', DomUtils.debounce(() => {
            this.saveFormData();
        }, 500));
    }
    ...
    static showError(field, message) {
        ...
        if (!errorElement) {
            errorElement = DomUtils.createElement('span', {
                id: errorId,
                class: ['form-error'],
                role: 'alert'
            });
...
class PerformanceManager {
    ...
    static preconnectResources() {
        ...
        links.forEach(link => {
            const preconnect = DomUtils.createElement('link', {
                rel: 'preconnect',
                href: link,
                crossorigin: ''
            });
...
class PortfolioApp {
    ...
    static initOptionalComponents() {
        ...
        if (testimonialSlides.length > 0) {
            TestimonialCarouselManager.init();
        }
    ...
    static nextTestimonial() {
        TestimonialCarouselManager.nextSlide();
    }

    static prevTestimonial() {
        TestimonialCarouselManager.prevSlide();
    }

    static filterPortfolio(category) {
        PortfolioHierarchyManager.switchCategory(category, true);
    }
    
    static filterBranding(subcategory) {
        PortfolioHierarchyManager.switchBrandingSubcategory(subcategory, true);
    }
}
